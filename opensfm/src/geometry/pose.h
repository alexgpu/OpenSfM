  ...
  for (int i = 0; i < 2; ++i) {
    ...
    for (int j = 0; j < 2; ++j) {
      ...
      for (IT it = begin; it != end; ++it) {
        ...
        const Eigen::Vector3d point = geometry::TriangulateBearingsMidpointSolve(centers, bearings);
...
struct RelativePoseCost {
  ...
  bool operator()(const T* const parameters, T* residuals) const {
    ...
    // Eigen::Matrix<T, 3, -1> bearings(3 , 2);
    // Eigen::Matrix<T, 3, -1> centers(3 , 2);
    // centers.col(0) << Eigen::Matrix<T, 3, 1>::Zero();
    // centers.col(1) << translation;
    ...
    for (IT it = begin_; it != end_; ++it) {
      ...
      // bearings.col(0) << x;
      // ceres::AngleAxisRotatePoint(rotation_transpose.data(), y.data(), bearings.col(1).data());

      // // Triangulate
      // auto point = geometry::TriangulateBearingsMidpointSolve(centers, bearings);
      ...
      const auto n1 = x.cross(translation);
      const auto n2 = some_tmp.cross(minus_translation);

      

      residuals[0] += (n1.dot(y)+n2.dot(x))*T(0.5);
    }
...
Eigen::Matrix<double, 3, 4> RelativePoseRefinement(
  ...
  solver.options.max_num_iterations = 1;
